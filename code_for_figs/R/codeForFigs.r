...
makeFig1	<-	function(toFile=FALSE)
{
	...
	for(i in colnames(param))
	{
		...
		axis(side=2, at=seq(from=0, to=10, by=2)*1000, labels=c(0,NA,'4,000',NA,'8,000',NA), padj=.5)
		...
		# Time bias on AA
		plot(12:120,aa.static, type='l', xlim=c(0,120), ylim=c(0,1), xlab=NA, ylab=NA, xaxt='n', bty='n')
...
makeFig2ab	<-	function(d=PEAdata, toFile=FALSE,scale.bars=TRUE)
{
	...
	if(toFile)	pdf(file='../R-generated figs/PEA data fig.pdf', width=4.5,height=h)
...
makeFig2d	<-	function(myData=maxConcData, toFile=FALSE, xl=c(0,144), yl=c(0,4))	{
	...
	plot(	myData$Time, myData$nl2, type='n',
			xlim=xl, ylim=yl,
				xlab=NA, 
				ylab=NA,
				main=NA,
...
makeFig2e1	<-	function(a=brCa_dyn[brCa_dyn$Treatment%in%c('Erlotinib','Lapatinib'),], toFile=FALSE)	{
	...
	if(toFile)	pdf(file='../R-generated figs/HCC1954 conc effect curves w DIP.pdf', width=5, height=2.5)
...
makeFig2e2	<-	function(a=brCa_static[brCa_static$Treatment!='BIBW2992',], toFile=FALSE)	{
	if(!toFile)	dev.new(width=5, height=2.5)
	if(toFile)	pdf(file='../R-generated figs/HCC1954 standard DRC.pdf', width=5, height=2.5)
	par(mfrow=c(1,2),font.lab=2, mar=c(3,1,1,1), oma=c(1,3,0,0))
	for(tx in sort(unique(a$Treatment)))	{
...
drcm	<-	makeMelCompare(nr=1,nc=4)	# needed to make Fig 3
makeFig3()
